<template>
    <div class="row">
        <div class="input-group">
            <input type="text" class="form-control" placeholder="Enter Purchase Id" aria-label="Recipient's username"
                aria-describedby="basic-addon2" v-model="this.search_query">
            <div class="input-group-append">
                <button type="button" class="btn btn-primary" @click="this.fetchOrders()">Search</button>
            </div>
        </div>
    </div>
    <table class="table">
        ...
        <tbody>
            <tr v-if="!this.loading && (this.orders.length > 0)" v-for="(order, index) in this.orders" :key="index">
                ...
                <td>
                    <a href="#" v-if="order.status == 1" class="btn btn-sm btn-danger"
